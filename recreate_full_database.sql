...
-- INLUYE: TABLAS, RELACIONES, POLÍTICAS RLS (v7) Y DATOS INICIALES
-- =====================================================

-- 1. LIMPIEZA TOTAL (¡CUIDADO!)
-- Descomentar estas líneas si se desea borrar todo antes de empezar
...
CREATE TABLE IF NOT EXISTS public.categorias (
    ...
    empresa_id UUID REFERENCES public.empresas(id) ON DELETE CASCADE,
    nombre TEXT NOT NULL,
    tipo TEXT DEFAULT 'restaurante', -- restaurante, inventario, etc
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now())
);
...
ALTER TABLE public.ingresos_stock ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.detalle_ingresos_stock ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.kardex ENABLE ROW LEVEL SECURITY;

-- POLÍTICAS PARA PROFILES (Usa funciones para evitar recursión)
CREATE POLICY "profiles_select" ON public.profiles FOR SELECT TO authenticated USING (id = auth.uid() OR public.is_platform_admin() OR (empresa_id = public.get_my_empresa_id() AND public.is_oficina()));
CREATE POLICY "profiles_insert" ON public.profiles FOR INSERT TO authenticated WITH CHECK (id = auth.uid() OR public.is_platform_admin() OR (empresa_id = public.get_my_empresa_id() AND public.is_oficina()));
CREATE POLICY "profiles_update" ON public.profiles FOR UPDATE TO authenticated USING (id = auth.uid() OR public.is_platform_admin() OR (empresa_id = public.get_my_empresa_id() AND public.is_oficina()));
CREATE POLICY "profiles_delete" ON public.profiles FOR DELETE TO authenticated USING (public.is_platform_admin() OR (empresa_id = public.get_my_empresa_id() AND public.is_oficina()));

-- POLÍTICAS PARA EMPRESAS
CREATE POLICY "empresas_select" ON public.empresas FOR SELECT TO authenticated USING (public.is_platform_admin() OR id = public.get_my_empresa_id());
CREATE POLICY "empresas_update" ON public.empresas FOR UPDATE TO authenticated USING (public.is_platform_admin() OR (id = public.get_my_empresa_id() AND public.is_oficina()));

-- POLÍTICAS GENERALES (Filtrar por empresa_id)
-- Para simplificar, aplicamos una política genérica a las demás tablas
DO $$ 
DECLARE 
    tbl TEXT;
BEGIN
...
-- Superadmin (Nota: El ID debe coincidir con el de auth.users si se crea manualmente)
-- INSERT INTO public.profiles (id, empresa_id, nombre, email, rol)
-- VALUES ('TU_UID_DE_SUPABASE_AUTH', '00000000-0000-0000-0000-000000000001', 'Super Administrador', 'admin@billennium.com', 'admin_plataforma');
